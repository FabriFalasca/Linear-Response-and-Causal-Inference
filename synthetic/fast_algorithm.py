...
def compute_quantile_analytical_tau_discrete(x_t,phi,sigmas,tau,standardized='yes', q=[0.01,0.99]):
    N, T = x_t.shape
...
if __name__ == "__main__":
    ...
    print('\nAnalytical estimate integral:')
    x, y = driver(fun=compute_quantile_analytical_tau)
    print('Error: ', norm(x,x0), norm(y,y0))

    print('\nAnalytical estimate discrete:')
    x, y = driver(fun=compute_quantile_analytical_tau_discrete)
    print('Error: ', norm(x,x0), norm(y,y0))

    print('\n\n ------------------------')
    print('Now compute response for non-standardized time series')
    
    print('\nOriginal code:')
    x0, y0 = driver(fun=lambda *x: compute_quantile_original(*x, standardized='no'))

    print('\nAnalytical estimate integral:')
    x, y = driver(fun=lambda *x: compute_quantile_analytical_tau(*x, standardized='no'))
    print('Error: ', norm(x,x0), norm(y,y0))
